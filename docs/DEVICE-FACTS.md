...
`loop/list` 里原本两个 `GENERAL` 天气槽只剩一个（`REDACTED` 带地理位置的
...
## 架构决策：单一画板槽 + 自控节奏，不依赖设备自转（已验证，2026-07-31）

**背景**：`GET /loop/list` 目前只有 1 个 CANVAS_API 槽（`A_0i9VsgJ8Be`）。如果
6+ 张 Canvas 卡都写这一个槽，会互相覆盖；而设备原生轮转（`interval.powerMs`）
又会在我们没推送的时候自己把画面转走，两个问题叠在一起。

**实测验证的解法**：`POST /settings` 把 `interval.powerMs` 调到官方上限
`43200000`（12 小时）后，画面在 70 秒内（超过原 60 秒周期一整轮）纹丝不动。
设置变更有一次性的"残留跳转"（改设置后仍会完成一次已经排定的旧周期切换，
之后才按新周期生效），过了这一次就稳定。

**结论：** 不用去 App 里申请多个 Canvas 槽位。架构定为——**一个画板槽，
所有 Canvas 卡都是这个槽在不同时刻被我方服务写入的"视图"**，节奏完全由
`scheduler.py`（M6）显式控制，设备侧调到最大间隔只是为了防止它在我们
没推送的间隙自说自话地转走。这个决定同时也回答了"6 张卡怎么共用 1 个槽"
的问题：不是共时性展示，是分时复用。
