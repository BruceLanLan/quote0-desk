...
`key=REDACTED`（带地理位置，已移除）、`key=REDACTED`。
...
## P2：NFC 贴一下打开的确实是当前内容的 link（已确认，2026-08-04，项目立足点验证通过）

真机实测，用户配合贴了两次手机：

1. 第一次推 Text API 内容，`link` 指向 `http://<LAN_IP>:5252/t/ping`
   （当时本机局域网 IP），贴一下后用户报告手机打开的地址正是这个自定义
   IP:端口——但这个 IP 是 3 天前的，本机重新联网后已经换成
   `<LAN_IP>`，所以请求实际没能到达服务器（服务器日志没有 `/t/ping`
   记录，只有本机自己 curl 探测的痕迹）。
2. 改推 `link` 为修正后的 `http://<LAN_IP>:5252/t/ping`，用户再贴一次，
   手机打开的地址跟着变成了 `.58` 结尾——服务器日志实锤：

   ```
   2026-08-04 00:29:12,023 INFO ping hit
   2026-08-04 00:29:12,023 INFO <LAN_IP> - - [04/Aug/2026 00:29:12] "GET /t/ping HTTP/1.1" 200 -
   ```

**结论：** 两次贴手机，打开的地址都精确跟随当时推送内容里设置的 `link`
值变化——不是设备/App 里固定的地址，是"当前显示内容"绑定的那个 URL。
NFC 反馈闭环的核心假设成立，项目立足点验证通过。

顺带确认：Text API 的 `link` 字段是好使的（跟 `daviddwlee84/Quote0` 那条
"Image API 的 link 不工作"的社区反馈不冲突——那是另一种 API 类型的问题，
本项目当前架构里 Canvas 已出局、Text API 是主力文字通道，这条验证已经够）。

## M2：NFC 完整闭环真机验证通过（2026-08-04）——根因是手机自己开着 VPN

P2 确认"link 会跟着推送内容变"之后，测完整闭环（贴一下 → 服务器收到 →
计数器 +1 → 推回屏幕 → 屏幕真的变）时反复失败，中间排查绕了不少弯路，
记下来避免以后重蹈：

**弯路 1——每张 NFC 卡的 `link` 字段一直是空字符串。** proverb/todo/pet/
qiantong 从 M2/M6 起 `link` 就是 `""` 占位符没真正填过；而且 Text API
每次重推都是整份覆盖，不是增量 merge——服务端手动重推（比如 counter_tap
路由自己拼 `push_text()`）如果不显式带 `link`，等于把已经生效的 link
覆盖回空。这是真实代码 bug，已修（`config.nfc_base_url()` 统一收口，
各卡自己拼 `/t/...`，见对应 commit）。

**弯路 2——这台 Mac 局域网 IP 在一小时内变了两次（`.31→.58→.59`），且
同时挂着 10 个 `utun` 隧道接口 + 一个本地 SOCKS 代理。** 一度怀疑是 Mac
侧 VPN 拦截了局域网入站流量，为绕开它装了 `cloudflared` 建了个免注册的
公网隧道（`trycloudflare.com`），验证过隧道本身工作正常（自己 curl 能
通）。**这个怀疑最后被证伪**——见下面真正的根因。这个隧道方案本身没有
问题，留着当"手机和 Mac 不在同一局域网"场景的备用方案，只是不是这次
真正卡住的原因。

**真正的根因——手机自己开着 VPN。** 用户在自己手机上关掉 VPN 之后，
同一个 `link`（`cloudflared` 的公网地址）立刻贴一下就成功了，服务器
日志实锤：

```
2026-08-04 01:44:11,241 INFO counter_tap -> n=5 push={'ok': True, ...}
2026-08-04 01:44:12,051 INFO 127.0.0.1 - - [04/Aug/2026 01:44:12] "GET /favicon.ico HTTP/1.1" 404 -
```

（`favicon.ico` 请求是移动端浏览器的正常行为，证明这次是真的跳进了
网页，不是停在 App 预览。）关 VPN 之前的失败表现是"NFC 跳到 Dot App
内部一个小窗口预览，没有转发到我们的 link，App 内没有任何可点的东西"——
这跟"网络不通"的表现完全不同，早点意识到"App 内部预览 = 手机侧拦截/
接管了这次跳转"，能少走前两条弯路。

**结论：** M2 全链路（NFC 贴一下 → 服务器动作 → 屏幕刷新）真机验证通过。
以后如果 NFC 又出现"跳到 App 而不是网页"的症状，先问用户手机是否开着
VPN，这是已知的、验证过的头号嫌疑对象，不用再重新走一遍网络排查。

## 待验证

- P3：`link` 支持哪些 scheme（公网 https / `shortcuts://`；局域网 http 已确认可行，见上）
